fetch('https://v6.exchangerate-api.com/v6/822304e8ee8183e9de49f5df/latest/usd')
    .then(async (response) => {
        ...
        document.getElementById('last-updated').innerHTML = `${new Date(data.time_last_update_unix * 1000).toLocaleDateString('en-US')}, ${new Date(data.time_last_update_unix * 1000).toLocaleTimeString('en-US')}`; // prettier-ignore
        document.getElementById('next-update').innerHTML = `${new Date(data.time_next_update_unix * 1000).toLocaleDateString('en-US')}, ${new Date(data.time_next_update_unix * 1000).toLocaleTimeString('en-US')}`; // prettier-ignore
