...
if (fs.existsSync(path.join(dirname, "public", "data")))
  fs.rmSync(path.join(dirname, "public", "data"), { recursive: true });
fs.mkdirSync(path.join(dirname, "public", "data"));
fs.readdirSync("data").filter((file) => file.endsWith(".js")).forEach((file) => fs.copyFileSync(path.join(dirname, "data", file), path.join(dirname, "public", "data", file)));
fs.readdirSync("public/scripts").forEach((fileOrFolder) => {
  if (fileOrFolder.endsWith("external"))
    return;
  if (fileOrFolder.endsWith(".js"))
    fs.rmSync(path.join(dirname, "public", "scripts", fileOrFolder));
  else
    fs.rmSync(path.join(dirname, "public", "scripts", fileOrFolder), { recursive: true });
});
fs.mkdirSync(path.join(dirname, "public", "scripts", "pages"));
fs.readdirSync("scripts").forEach((fileOrFolder) => {
  if (fileOrFolder.endsWith(".js"))
    fs.copyFileSync(path.join(dirname, "scripts", fileOrFolder), path.join(dirname, "public", "scripts", fileOrFolder));
});
fs.readdirSync("scripts/pages").forEach((category) => {
  ...
  fs.readdirSync(path.join(dirname, "scripts", "pages", category)).forEach((file) => {
    if (file.endsWith(".js"))
      fs.copyFileSync(path.join(dirname, "scripts", "pages", category, file), path.join(dirname, "public", "scripts", "pages", category, file));
  });
});
      ...
      if (!coinsDatabaseEntry) {
        coinsDatabaseEntry = coinsModel.create({
          ...
          coins: coinType.coins?.map((variant) => ({
            ...variant,
            coins: variant.coins?.map((coin) => ({ ...coin, id: Math.floor(Math.random() * 9e9 + 1e9) }))
          }))
        });
      }
...
fastify.post("/coins-list-edit", async (request, reply) => {
  ...
  if (password !== process.env.COINS_PASSWORD)
    return reply.send(JSON.stringify({ error: "Invalid password!" }, null, 2));
  const databaseCoinType = await coinsModel.findOne({ id: coinTypeId });
  if (!databaseCoinType)
    return reply.send(JSON.stringify({ error: "Invalid coin type!" }, null, 2));
...
fastify.get("/calendar-events", async (request, reply) => {
  if (calendarEventsCache)
    return reply.send(calendarEventsCache);
  const holidays = (await (await fetch(`https://www.googleapis.com/calendar/v3/calendars/en.usa%23holiday%40group.v.calendar.google.com/events?key=${process.env.GOOGLE_CALENDAR_API_KEY}`)).json()).items.map((holiday) => ({ name: holiday.summary, date: holiday.start.date })).filter((holiday) => !holiday.name.includes(" (substitute)")).sort((a, b) => new Date(a.date) - new Date(b.date));
  const moonPhases = (await (await fetch(`https://www.googleapis.com/calendar/v3/calendars/ht3jlfaac5lfd6263ulfh4tql8%40group.calendar.google.com/events?key=${process.env.GOOGLE_CALENDAR_API_KEY}`)).json()).items.map((moonPhase) => ({
...
fastify.post("/calendar-todo-edit", async (request, reply) => {
  ...
  if (password !== process.env.CALENDAR_TODO_PASSWORD)
    return reply.send(JSON.stringify({ error: "Invalid password!" }, null, 2));
  if (isNaN(year) || isNaN(month) || isNaN(date))
    return reply.send(JSON.stringify({ error: "A date parameter is NaN!" }, null, 2));
  if (month < 1 || month > 12)
    return reply.send(JSON.stringify({ error: "Invalid month parameter!" }, null, 2));
  if (date < 1 || date > 31)
    return reply.send(JSON.stringify({ error: "Invalid date parameter!" }, null, 2));
  ...
  if (!yearEntry.dates)
    yearEntry.dates = {};
  if (!yearEntry.dates[month])
    yearEntry.dates[month] = {};
  yearEntry.dates[month][date] = todo;
  await todoModel.replaceOne({ year }, yearEntry);
  const data = Object.fromEntries((await todoModel.find({})).map((todo2) => [todo2.year, todo2.dates]));
  if (!todoOptions)
    todoOptions = (await todoOptionsModel.findOne({})).data;
  reply.send(JSON.stringify({ todo: todoOptions, data }, null, 2));
});
...
fastify.get(
  ...
  (request, reply) => reply.send(
    JSON.stringify(
      Object.values(pagesParsed).map((category) => Object.values(category)).flat().map((page) => ({ title: page.title, id: page.id, category: page.category, link: page.link, description: page.descriptionParsed, keywords: page.keywords })),
      null,
      2
    )
  )
...
fastify.get("/cors-anywhere", async (request, reply) => {
  ...
  if (response.headers.get("content-type").startsWith("image/"))
    reply.header("Access-Control-Allow-Origin", "*").type("image/png").status(response.status).send(Buffer.from(await response.arrayBuffer()));
  else
    reply.header("Access-Control-Allow-Origin", "*").status(response.status).send(await response.text());
});
const pagesWithScripts = fs.readdirSync("scripts/pages").map(
  (category) => fs.readdirSync(`scripts/pages/${category}`).filter((script) => script.endsWith(".js")).map((script) => `${category}/${script.replace(/\.js$/, "")}`)
).flat();
const pagesWithStyles = fs.readdirSync("public/styles/pages").map((category) => fs.readdirSync(`public/styles/pages/${category}`).map((style) => `${category}/${style.replace(/\.css$/, "")}`)).flat();
let totalCategories = 0, totalPages = 0;
fs.readdirSync("views/pages").forEach((category) => {
  ...
  pages.forEach((page) => {
    ...
    if (!pageInfo)
      return consola.log(`${chalk.blue("[Page Auto-Loader]")} ${chalk.red(`Unable to find page information for ${category}/${page}!`)}`);
...
fastify.listen({ port, host: "0.0.0.0" }, (error) => {
  if (error) {
    if (error.code === "EADDRINUSE")
      ...
      consola.error(`${chalk.red("[Startup error]:")} ${error}`);
    process.exit(1);
  }
