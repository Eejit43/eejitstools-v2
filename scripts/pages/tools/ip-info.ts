...
fetch('https://ipgeolocation.abstractapi.com/v1/?api_key=6b7d5eb7b17e420fbd0f8733e4b726fa').then(async (response) => {
    ...
    (document.querySelector('#ip-address') as HTMLSpanElement).innerHTML = `${data.ip_address} (${data.security.is_vpn ? 'Is' : 'Not'} a known <span class="tooltip-bottom" data-tooltip="Virtual private network">VPN</span>)`;
    (document.querySelector('#isp') as HTMLSpanElement).textContent = `${data.connection.isp_name} ${data.connection.isp_name === data.connection.organization_name ? '' : `(${data.connection.organization_name}) `}(${data.connection.connection_type})`;
