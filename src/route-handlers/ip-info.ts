...
export default function setupIpInfoRoute(fastify: FastifyInstance) {
    fastify.get('/ip-info', async (request, reply) => {
        ...
        if (ip === '127.0.0.1') {
            ...
            const parsedInformation = Object.fromEntries(
                cloudflareInformation.split('\n').map((information) => information.split('=') as [string, string]),
            );
        ...
        const ipInformation = await (
            await fetch(`https://ipgeolocation.abstractapi.com/v1/?api_key=${process.env.ABSTRACT_API_API_KEY}&ip_address=${ip}`)
        ).text();
